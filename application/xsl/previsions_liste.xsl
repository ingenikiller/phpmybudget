...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="ANNEE">
		 <xsl:choose>
		 	...
		 	<xsl:otherwise><xsl:value-of select="/root/request/annee"/></xsl:otherwise>
		 </xsl:choose>
	 </xsl:param>
	<!-- template js -->
	...
	<xsl:template name="Contenu">
		...
		<center>
			<table>
				<tr>
					...
					<select name="annee" id="annee" onchange="refreshWindow()">
						...
						<!--option value="2012">
							<xsl:if test="$ANNEE='2012'">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>			
							<xsl:value-of select="'2012'"/></option>
						<option value="2013">
							<xsl:if test="$ANNEE='2013'">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>
							<xsl:value-of select="'2013'"/>
						</option>
						<option value="2014">
							<xsl:if test="$ANNEE='2014'">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>
							<xsl:value-of select="'2014'"/>
						</option>
						<option value="2015">
							<xsl:if test="$ANNEE='2015'">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>
				...
				<tr>
					<td>
						<input type="button" id="" name="" value="Entete" onclick="afficheEntete('{$NUMEROCOMPTE}');"/>
						<select name="listeEntete" id="listeEntete" onchange="afficheListeGroupe(this.value)"/>
					</td>
				</tr>
				<tr>
					<td>
						<input type="button" id="" name="" value="Unitaire" onclick="afficheUnitaire('{$NUMEROCOMPTE}','');"/>
					
					</td>
				</tr>
				<!--<input type="hidden" id="annee" name="annee" value="{/root/data/ListePeriodes/Periode[1]/annee}"/>-->
			...
			<table>
				<tr>
					<td>Solde</td>
					<td><xsl:value-of select="format-number(number(/root/data/SommeOperations/Dynamic/total) + number(/root/data/Comptes/solde), $FORMAT_MNT)"/></td>
				</tr>
				<tr>
					<td>Estimation</td>
					<td>
	...
	<xsl:template name="boiteUnitaire">
			...
			<form method="POST" name="editionPrevisionUnitaire" id="editionPrevisionUnitaire" action="index.php" onsubmit="return modifierPrevision(this)">
				...
				<table>
					...
					<tr>
						...
						<td>
							<input size="7" name="montant" id="montant" class="numerique_obligatoire" onblur="return isDouble(this);" tabindex="15"/>
						</td>
					</tr>
					<tr>
		...
		<div id="boiteEntete" title="Edition" style="display: none;">
			<!--
				formulaire entete
			-->
			<form method="POST" name="editionEnteteUnitaire" id="editionEnteteUnitaire" action="index.php" onsubmit="return creerEntete(this)">
				<input type="hidden" id="numeroCompte" name="numeroCompte" value="{$NUMEROCOMPTE}"/>
